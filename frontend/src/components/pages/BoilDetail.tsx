...
const BoilDetail: React.FC = (): JSX.Element => {
  ...
  const tabs = (id: string) => {
    switch (id) {
      case "Summary":
        return data.summaryRows.length === 0 ? <NoDataHandler /> : <SummaryTable items={data.summaryRows} />;

      case "Weighting":
        return data.weightingRows.length === 0 ? <NoDataHandler /> : <WeightingTable items={data.weightingRows} />;
      case "Load":
        return data.loadRows.length === 0 ? <NoDataHandler /> : <LoadTable items={data.loadRows} />;
      case "Technology":
        return data.technologyRows.length === 0 ? <NoDataHandler /> : <TechnologyTable items={data.technologyRows} />;
    }
  };

  if (error) {
    return <div className={classes.centeredMessage}>Error...</div>;
  }
    ...
    <div className={classes.pageContainer}>
      <div className={classes.pageSpaceHeader}>
        ...
        <div className={classes.pageSubHeader}>
          <Link className={classes.tableLink} to={`${RouteNames.BOIL_PDF}/${boil_id}`}>
      ...
      <div className={classes.nestedNavigation}>
        ...
        <button
          className={tab === "Technology" ? classes.navButtonActive : classes.navButton}
          id={"Technology"}
          onClick={(e) => setTab(e.currentTarget.id)}
        >
          Тех.карта
      ...
      <div className={classes.pageTableContainer}>{tabs(tab)}</div>
    </div>
  );
};
