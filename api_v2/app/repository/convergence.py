...
class ConvergenceRepository:
    ...
    def __plant_options(self):
        plants = db.session.query(
            Plant.PlantAlias.label("key"), Plant.PlantName.label("value")
        ).all()
        plant_options = self.plant_schema.dump(plants, many=True)
        return plant_options

    def __query(self, options: ConvergenceRequestOptions):

        filters = []
        filter = options.filter
        filters.append(Weighting.Quantity.is_(None))
        filters.append(Batch.BatchDate.between(filter.start_date, filter.end_date))
        if filter.plant != "-":
        ...
        report_subquery = (
            ...
            .join(Batch, Batch.BatchPK == Boil.BatchPK)
            .join(Plant, Plant.PlantAlias == Batch.Plant)
            .join(BtProduct, BtProduct.BatchPK == Boil.BatchPK)
            .join(Product, Product.ProductId == BtProduct.ProductId)
            .join(
                Weighting,
                (
    ...
    def __query_ex(self, options: ConvergenceRequestOptions):
        filters = []
        filter = options.filter
        ...
        query = (
            ...
            .distinct(report_subquery.c.batch_id)
            .order_by(
                report_subquery.c.batch_year,
                report_subquery.c.batch_month,
                report_subquery.c.batch_number,
    ...
    def __rows(self, query, options: ConvergenceRequestOptions):
