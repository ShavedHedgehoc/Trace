...
from app.schemas.boil import BoilItemHeaderSchema, BoilItemRowSchema, BoilItemSummaryRowSchema
...
class BoilItemRepository:
    ...
    def __header(self, boil_id: int):
        data = db.session.query(
            ...
            BtProduct, BtProduct.BatchPK == Batch.BatchPK, isouter=True
        ).join(
            Product, Product.ProductId == BtProduct.ProductId, isouter=True
        ).filter(
    ...
    def __load_query(self, id: int):
        query = db.session.query(
            ...
            Weighting, Weighting.ContainerPK == Load.ContainerPK
        ).join(
            Product, Product.ProductId == Weighting.ProductId
        
        ).join(
            Lot, Lot.LotPK == Weighting.LotPK
        ).join(
            Document, Document.DocumentPK == Load.DocumentPK
        ).join(
            Author, Author.AuthorPK == Document.AuthorPK
        ).filter(
            Load.BatchPK == id
        ).order_by(
            Product.ProductName
