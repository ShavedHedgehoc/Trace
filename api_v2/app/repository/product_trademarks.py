...
from app.schemas.product import ProductTrademarkItemHeaderSchema, ProductTrademarkItemRequestSchema, ProductTrademarkItemRowSchema
...
class ProductTrademarksRepository():
    ...
    def __subquery(self, id: str):
        product_sbqry = db.session.query(
            ...
            Plant, Plant.PlantName == Batch.Plant, isouter=True
        ).join(
    ...
    def __header(self, id: str):
        data = db.session.query(
            ...
            Product.ProductId == id).one_or_none()
        header = self.header_schema.dump(data)
        return header

    def __rows(self, query: BaseQuery, options: ProductTrademarksItemRequestOptions):
