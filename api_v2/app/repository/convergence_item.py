...
from app.schemas.convergence import (
    ConvergenceItemRequestSchema,
    ConvergenceItemRowSchema,
)
...
class ConvergenceItemRepository:
    ...
    def __batch_id(self, name: str) -> Optional[int]:
        batch = db.session.query(Batch).filter(Batch.BatchName == name).one_or_none()
        return batch.BatchPK if batch else None

    def __query(self, options: ConvergenceItemRequestOptions, id: int):
        ...
        if options.exactly:
            filters.append(
                or_(
                    wght_subqry.c.fact != boil_subqry.c.plan,
